...
public class ChucNangController {
    ...
    private final ChucNangService service;
    ...
    @GetMapping("/{id}")
    public ResponseEntity<ChucNang> getById(@PathVariable Integer id) {
        return service.getById(id).map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
    }

    @PostMapping
    public ResponseEntity<ChucNang> create(@RequestBody ChucNang entity) {
        return ResponseEntity.status(HttpStatus.CREATED).body(service.create(entity));
    }

    @PutMapping("/{id}")
    public ResponseEntity<ChucNang> update(@PathVariable Integer id, @RequestBody ChucNang entity) {
